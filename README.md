...
| 局限 | 描述 | 影响 |
|------|------|------|
| 单手支持 | 当前仅支持单手交互 | 限制了双手协作的创作可能 |
| 光线敏感 | 低光环境下识别准确率下降 | 需要良好的照明条件 |
| 音频本地化 | 音频在服务端播放，Web 端仅接收视频 | 网络延迟影响体验 |
| 无 MIDI 导出 | 项目数据为 JSON，未实现标准 MIDI 导出 | 不便于导入专业 DAW |

### 7.2 未来改进方向

| 方向 | 描述 | 优先级 |
|------|------|-------|
| **双手交互** | 支持双手同时绘画，左右手分配不同乐器 | 高 |
| **Web Audio API** | 将音频合成迁移到浏览器端，消除网络延迟 | 高 |
| **MIDI 导出** | 支持导出标准 MIDI 文件，便于专业后期处理 | 中 |
| **AI 伴奏** | 基于当前旋律自动生成和弦进行与伴奏 | 中 |
| **多人协作** | 支持多用户同时绘画，创作合奏作品 | 低 |
| **移动端优化** | 优化触屏体验，支持手指绘画模式 | 低 |
